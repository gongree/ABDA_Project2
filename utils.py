...
def pcc_to_adjacency(pcc_fc, top_percent=30):
    batch, node, _ = pcc_fc.shape
    adjacency_matrices = torch.zeros_like(pcc_fc, dtype=torch.int)  # Initialize binary adjacency matrices

    for b in range(batch):
        # Flatten the upper triangle of the PCC matrix (excluding diagonal)
        triu_indices = torch.triu_indices(node, node, offset=1, device=pcc_fc.device)  # Indices for the upper triangle
        upper_tri_values = pcc_fc[b][triu_indices[0], triu_indices[1]]  # Get values in the upper triangle
        
        # Determine the threshold for the top `top_percent`
        threshold = torch.quantile(upper_tri_values, 1 - top_percent / 100)
        
        # Create a binary mask for values above the threshold
        mask = pcc_fc[b] >= threshold
        
        # Ensure symmetry and zero diagonal
        adjacency = torch.where(mask, 1, 0)
        adjacency.fill_diagonal_(0)
        
        # Store the result
        adjacency_matrices[b] = adjacency
